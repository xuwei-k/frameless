...
/**
  ...
  */
trait SelectorByValue[L <: HList, Value] extends DepFn1[L] with Serializable { type Out <: String }

object SelectorByValue {
  type Aux[L <: HList, Value, Out0 <: String] = SelectorByValue[L, Value] { type Out = Out0 }

  implicit def select[K <: String, T <: HList, Value](implicit wk: Witness.Aux[K]): Aux[FieldType[K, Value] :: T, Value, K] = {
